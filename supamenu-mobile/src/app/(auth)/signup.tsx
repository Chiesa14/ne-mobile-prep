    ...
    <SafeAreaView className="bg-white rounded-t-xl  px-6  flex flex-1 h-screen">
      <ScrollView showsVerticalScrollIndicator={false}>
        <Pressable onPress={() => router.push("/")}>
          <Animated.View
            key={"UniqueKey102"}
            entering={FadeInLeft.duration(1000).easing(Easing.in(Easing.ease))}
            className="flex fixed flex-row justify-center pt-10 items-center"
          >
            <Image
            ...
            <Text className="text-xl font-PoppinsBold text-primary">
              Supa<Text className="text-black font-PoppinsSemiBold">Menu</Text>
            </Text>
          </Animated.View>
        </Pressable>
        <AuthHeader
          title="Enjoy now,"
          description="Feel free to enjoy, order best meals !"
        />
        <Animated.View
          key={"UniqueKey102"}
